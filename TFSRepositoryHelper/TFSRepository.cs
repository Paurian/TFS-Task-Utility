...
{
    public class TFSRepository // Warning CA1001  Implement IDisposable on 'TFSRepository' because it creates members of the following IDisposable types: 'TfsTeamProjectCollection'.
    {
        ITFSSettings settings;

        public TFSRepository(ITFSSettings settings)
        {
        ...
        public WorkItemCollection GetCollectionOfTasks(WorkItemStore workItemStore, ITFSSettings tfsSettings)
        {
            if (String.IsNullOrEmpty(tfsSettings.TfsProject))
            {
            ...
            var tfsArea = String.Format("{0}\\{1}", tfsSettings.TfsProject, tfsSettings.TfsTeam);
            var projectAreas = GetAreas(tfsSettings.TfsServer, tfsSettings.TfsWorkStore, tfsSettings.TfsProject);
            if (!projectAreas.Contains(tfsArea))
            {
                tfsArea = tfsSettings.TfsProject;
            }

            var tfsIteration = tfsSettings.TfsIteration;

            if (String.IsNullOrEmpty(tfsSettings.TfsIteration))
            {
                tfsIteration = tfsSettings.TfsProject;
            }
            ...
            queryBuilder.Append("SELECT " +
                                ...
                                "FROM " +
                                  "WorkItemLinks " +
                                "WHERE " +
                                  "[Source].[System.WorkItemType] IN ('Task') AND ");
            ...
            queryBuilder.AppendFormat("[Source].[System.State] <> 'Removed' AND " +
                                      "[Source].[System.IterationPath] = '{0}' AND ", tfsIteration);

            queryBuilder.Append("[System.Links.LinkType] <> '' ");

            var q = new Query(workItemStore, queryBuilder.ToString());
            var wili = q.RunLinkQuery().Where(w => w.SourceId == 0); // SourceID = 0 means we get the leaf nodes, which should all be tasks.
        ...
        public WorkItemCollection GetListOfBacklogItemsWithoutWork(WorkItemStore workItemStore, ITFSSettings tfsSettings, INewTaskSettings newTaskSettings) // string tfsAreaPath, string tfsIterationPath, string newTaskExceptionFilterValue, string newTaskStoryExceptionFilterValue) settings.TfsArea, settings.TfsIteration, settings.NewTaskExceptionFilter, settings.NewTaskStoryExceptionFilter
        {
            ...
            if (!String.IsNullOrEmpty(newTaskSettings.NewTaskStoryExceptionFilter))
            {
                queryBuilder.AppendFormat("{0} AND ", GetCondition(newTaskSettings.NewTaskStoryExceptionFilter, "[Source].[System.Title]", "NOT CONTAINS", "AND"));
            }

            queryBuilder.Append("[System.Links.LinkType] <> '' ");

            if (!String.IsNullOrEmpty(newTaskSettings.NewTaskExceptionFilter))
            {
                queryBuilder.Append("AND " +
                                    "( " +
                                        "[Target].[System.WorkItemType] = 'Task' AND " +
                                        GetCondition(newTaskSettings.NewTaskExceptionFilter, "[Target].[System.Title]", "CONTAINS", "OR") +
                                    " ) ");
            }
        ...
        private string GetFieldValue(WorkItem workItem, string fieldName, string defaultResult = "")
        {
            return workItem.Fields.Contains(fieldName) ? workItem.Fields[fieldName].Value?.ToString() : defaultResult;
        }

        public List<IBackLogItem> GetBackLogItems(WorkItemCollection workItemCollection)
        {
            List<IBackLogItem> backLogItems = new List<IBackLogItem>();
            foreach (WorkItem workItem in workItemCollection)
            {
                var assignedTo = GetFieldValue(workItem, "Assigned To", "Unassigned");
                int estimatedEffort = 0;
                Int32.TryParse(GetFieldValue(workItem, "Effort"), out estimatedEffort);
                var acceptanceCriteria = GetFieldValue(workItem, "Acceptance Criteria");
                ...
                backLogItems.Add(new BackLogItem()
                {
                    ...
                    BackLogItemState = workItem.State,
        ...
        public List<ITaskItem> GetTaskItems(WorkItemCollection workItemCollection)
        {
            List<ITaskItem> storyItems = new List<ITaskItem>();
            foreach (WorkItem workItem in workItemCollection)
            {
                var activityType = GetFieldValue(workItem, "Activity");
                var assignedTo = GetFieldValue(workItem, "Assigned To", "Unassigned");
                var isBlocked = GetFieldValue(workItem, "Blocked") == "Yes";
                int estimatedEffort = 0;
                Int32.TryParse(GetFieldValue(workItem, "Estimated Hours"), out estimatedEffort);
                int actualEffort = 0;
                Int32.TryParse(GetFieldValue(workItem, "Total Hours"), out actualEffort);
                int remainingEffort = 0;
                Int32.TryParse(GetFieldValue(workItem, "Remaining Work"), out remainingEffort);
                var notes = GetFieldValue(workItem, "Notes");

                storyItems.Add(new TaskItem()
                {
                    ID = workItem.Id,
                    Title = workItem.Title,
                    IterationPath = workItem.IterationPath,
                    TaskState = workItem.State,
                    Blocked = isBlocked,
                    AssignedTo = assignedTo,
                    EstimatedEffort = estimatedEffort,
        ...
        private int BuildAndSaveSQATask(WorkItemType taskWorkItemType, WorkItem parentWorkItem, INewTaskSettings settings, WorkItemLinkTypeEnd hierarchyWorkItemLinkTypeEnd)
        {
        ...
        public List<int> AddTaskToEachBackLogItemInWorkItemCollection(WorkItemStore workItemStore, WorkItemCollection workItemCollection, ITFSSettings settings, Delegate buildTaskMethod, List<IBackLogItem> backLogItems)
        {
            Project teamProject = workItemStore.Projects[settings.TfsProject];
            ...
            foreach (WorkItem workItem in workItemCollection)
            {
                if (backLogItems.Any(t => t.ID == workItem.Id))
                {
        ...
        public List<int> AddTaskToEachBackLogItemInWorkItemCollection(WorkItemStore workItemStore, WorkItemCollection workItemCollection, ITFSSettings settings, List<IBackLogItem> backLogItems)
        {
            var result = AddTaskToEachBackLogItemInWorkItemCollection(workItemStore, workItemCollection, settings, new Func<WorkItemType, WorkItem, INewTaskSettings, WorkItemLinkTypeEnd, int>(BuildAndSaveSQATask), backLogItems);
            return result;
        }
